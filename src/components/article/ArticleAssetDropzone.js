...
export default function ArticleAssetDropzone({ articleId, assets, onAssetsChanged }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-500 font-mono">Add Assets to Article</h3>
      ...
      <div
        ...
        className={`w-full min-h-[160px] border-2 border-dashed rounded-2xl flex flex-col items-center justify-center p-6 text-center cursor-pointer transition-all duration-300 relative ${
          dragActive
            ? 'border-amber-500 bg-amber-500/5 dark:border-amber-500/30'
            : 'border-zinc-300 hover:border-[var(--accent)] dark:border-zinc-800 dark:hover:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-900/30'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">
          Drag & drop supplementary files here, or <span className="text-[var(--accent)] dark:text-blue-400 hover:underline">browse files</span>
        </p>
      ...
      {((assets && assets.length > 0) || Object.keys(uploadingFiles).length > 0) && (
        <div className="space-y-2 border border-zinc-200 dark:border-zinc-850 bg-white/40 dark:bg-zinc-900/10 p-4 rounded-xl">
              ...
              <div key={tempId} className="flex items-center justify-between py-3">
                <div className="flex items-center space-x-3 min-w-0">
                  <div className="p-2 bg-zinc-100 dark:bg-zinc-800/80 rounded-lg shrink-0">
                    {info.error ? <AlertCircle className="w-5 h-5 text-red-500 animate-pulse" /> : <Loader2 className="w-5 h-5 text-[var(--accent)] animate-spin" />}
                  </div>
                  <div className="min-w-0 text-left">
                    ...
                    <p className="text-[9px] text-zinc-500 font-mono uppercase tracking-widest">
                      {info.error ? <span className="text-red-500 font-bold">Failed to upload</span> : <span>Uploading ({formatBytes(info.size)})</span>}
                    </p>
                  </div>
                </div>
                <div>
            ...
            {assets && assets.map((asset) => (
              <div key={asset.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                <div className="flex items-center space-x-3 min-w-0">
                  ...
                  <div className="min-w-0 text-left">
                    <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 truncate">{asset.original_filename}</p>
                    <p className="text-[9px] text-zinc-500 font-mono uppercase tracking-widest">
