...
const Footer = () => {
  ...
  return (
    <footer className="bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-200/80 dark:border-zinc-800/60 mt-auto py-12 transition-premium text-left">
      <div className="w-full px-4 sm:px-8 lg:px-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
          ...
          <div className="flex flex-col space-y-4 sm:col-span-2 md:col-span-1">
            ...
            <p className="text-xs leading-relaxed text-zinc-505 dark:text-zinc-400 max-w-xs">
            ...
            <div className="flex items-center space-x-3 text-zinc-400 dark:text-zinc-500">
              <GraduationCap className="w-4 h-4 hover:text-zinc-850 dark:hover:text-zinc-200 cursor-pointer transition-colors" />
              <BookOpen className="w-4 h-4 hover:text-zinc-850 dark:hover:text-zinc-200 cursor-pointer transition-colors" />
              <FileText className="w-4 h-4 hover:text-zinc-850 dark:hover:text-zinc-200 cursor-pointer transition-colors" />
            </div>
          </div>

          {/* Columns 2-4: Dynamic Categories from Database */}
          {categories.length > 0 ? (
            categories.map((cat) => (
              <div key={cat.id}>
                <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 mb-4 font-serif">
                  {cat.name}
                </h3>
                <ul className="space-y-2.5 text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                  {cat.pages && cat.pages.map((p) => (
                    <li key={p.id}>
                      <Link href={`/${p.slug}`} className="hover:text-amber-500 transition-colors">
                        {p.title}
                      </Link>
                    </li>
                  ))}
            ...
            /* Fallback static placeholders if DB/API is not yet loaded */
            <>
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 mb-4 font-serif">
                  Resources
                ...
                <ul className="space-y-2.5 text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                  <li>
                    <Link href="/editorial-board" className="hover:text-amber-500 transition-colors">
                      Editorial Board
                    </Link>
                  </li>
                </ul>
              ...
              <div>
                ...
                <ul className="space-y-2.5 text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                  ...
                  <li>
                    <Link href="/manifests" className="hover:text-amber-500 transition-colors">
                      Metadata Manifests
                    </Link>
                  </li>
                </ul>
          ...
          {/* Column 5: Newsletter Subscriber */}
          <div className="flex flex-col space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 font-serif">
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleSubscribeSubmit} className="flex relative w-full">
                <input
                  ...
                  className="w-full text-xs font-medium pl-3 pr-8 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md focus:outline-none focus:border-zinc-450 dark:focus:border-zinc-700 transition-colors text-zinc-800 dark:text-zinc-200"
                />
                <button 
                  type="submit"
                  className="absolute right-1 top-1 p-1.5 bg-zinc-900 hover:bg-zinc-950 dark:bg-zinc-200 dark:hover:bg-white text-white dark:text-zinc-955 rounded-md transition-premium cursor-pointer"
                >
