...
import { Check, ChevronLeft, ChevronRight, Eye, FileText, Globe2, ImageOff, ImagePlus, LibraryBig, Newspaper, NotebookTabs, Pencil, X } from 'lucide-react';
...
import { formatAdvertisementPlacement, formatAdvertisementSchedule, formatAdvertisementStatus, formatAdvertisementTargetSummary } from './advertisingUtils';
...
const emptyForm = { title: '', alt_text: '', redirect_url: '', placement: 'sidebar_sticky', sidebar_side: '', priority: 0, status: 'draft', open_in_new_tab: true, starts_at: '', ends_at: '', image_media_id: null, target_area: '', target_mode: '', publication_type: '', publication_id: '', page_key: '', article_id: '' };
...
const toUtcIso = (value) => value ? new Date(value.includes('T') ? value : value.replace(' ', 'T')).toISOString() : null;
...
export default function AdvertisementWorkspace({ initialId = null }) {
  ...
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value, ...(key === 'placement' && value !== 'sidebar_sticky' ? { sidebar_side: '' } : {}) }));
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 4) {
      ...
      if (form.target_area === 'article' && form.placement === 'sidebar_sticky' && !['left', 'right'].includes(form.sidebar_side)) return 'Choose the left or right sidebar.';
  ...
  const save = async () => {
    ...
    const payload = { title: form.title, image_media_id: form.image_media_id, alt_text: form.alt_text || null, redirect_url: form.redirect_url || null, placement: form.placement, sidebar_side: form.target_area === 'article' && form.placement === 'sidebar_sticky' ? form.sidebar_side : null, priority: Number(form.priority), status: form.status, open_in_new_tab: form.open_in_new_tab, starts_at: toUtcIso(form.starts_at), ends_at: toUtcIso(form.ends_at), targets };
  ...
  return <div className="space-y-8">
    ...
    <AdvertisementList items={items} onEdit={beginEdit} onView={setViewingAd} />
    {viewingAd && <AdvertisementDetail ad={viewingAd} onClose={() => setViewingAd(null)} onEdit={() => { setViewingAd(null); beginEdit(viewingAd); }} />}
  </div>;
}
...
function DisplayStep({ form, update }) { return <div className="mx-auto max-w-3xl"><StepHeading title="Configure display settings" description="Control where the creative sits, what happens on click, and when it is visible." /><div className="space-y-7"><ChoiceCards label="Placement" value={form.placement} onChange={(value) => update('placement', value)} options={PLACEMENTS.map((item) => [item.id, item.label, item.description])} />{form.target_area === 'article' && form.placement === 'sidebar_sticky' && <ChoiceCards label="Sidebar Side" value={form.sidebar_side} onChange={(value) => update('sidebar_side', value)} options={[[ 'left', 'Left Sidebar', 'Appears below the dynamic article table of contents.' ], [ 'right', 'Right Sidebar', 'Appears in the dedicated article advertising rail.' ]]} />}<TextField label="Redirect URL" type="url" value={form.redirect_url} onChange={(value) => update('redirect_url', value)} placeholder="https://example.com/campaign" /><label className="flex items-center gap-3 rounded-xl border border-zinc-200 p-4 text-sm font-semibold dark:border-zinc-700"><input type="checkbox" checked={form.open_in_new_tab} onChange={(event) => update('open_in_new_tab', event.target.checked)} className="h-4 w-4" />Open redirect in a new tab</label><div className="grid gap-5 sm:grid-cols-2"><SelectField label="Status" value={form.status} onChange={(value) => update('status', value)} options={[[ 'draft', 'Draft' ], [ 'active', 'Active' ], [ 'inactive', 'Inactive' ]]} /><TextField label="Priority" type="number" value={form.priority} onChange={(value) => update('priority', value)} helper="Higher priority ads appear first when multiple ads use the same placement." /></div><div><div className="grid gap-5 sm:grid-cols-2"><DateTimeField label="Start date" value={form.starts_at} onChange={(value) => update('starts_at', value)} /><DateTimeField label="End date" value={form.ends_at} onChange={(value) => update('ends_at', value)} minDate={form.starts_at || undefined} /></div><p className="mt-2 text-xs text-zinc-500">Leave dates empty to show the ad whenever it is active.</p></div></div></div>; }

function ReviewStep({ destination, publications, pages, articles, target, form, previewUrl }) { const placement = PLACEMENTS.find((item) => item.id === form.placement)?.label || form.placement; return <div className="mx-auto max-w-3xl"><StepHeading title="Review advertisement" description="Confirm the destination, creative, and display rules before saving." />{previewUrl && <div className="mb-6 overflow-hidden rounded-2xl bg-zinc-950 p-4"><img src={previewUrl} alt={form.alt_text || form.title} className="mx-auto max-h-64 object-contain" /></div>}<div className="grid gap-4 sm:grid-cols-2"><Summary label="Destination" value={destination?.label} /><SummaryList label={destination?.type === 'journal' ? 'Selected Journals' : destination?.type === 'magazine' ? 'Selected Magazines' : 'Selected Website Pages'} values={publications.length ? publications.map((item) => `${destination.type === 'journal' ? 'Journal' : 'Magazine'} · ${item.title}`) : pages.map((item) => item.label)} /><Summary label="Target" value={target} />{pages.length > 0 && destination?.area !== 'website' && <SummaryList label="Selected Pages" values={pages.map((item) => item.label)} />}{articles.length > 0 && <SummaryList label="Selected Articles" values={articles.map((item) => item.title)} />}<Summary label="Creative" value={form.title} /><Summary label="Placement" value={placement} />{form.target_area === 'article' && form.placement === 'sidebar_sticky' && <Summary label="Sidebar side" value={`${form.sidebar_side} sidebar`} />}<Summary label="Status" value={form.status} /><Summary label="Redirect" value={form.redirect_url ? `${form.redirect_url}${form.open_in_new_tab ? ' · Opens in new tab' : ''}` : 'No redirect'} /><Summary label="Schedule" value={form.starts_at || form.ends_at ? `${form.starts_at || 'Immediately'} → ${form.ends_at || 'No end date'}` : 'Whenever active'} /></div></div>; }

function AdvertisementList({ items, onEdit, onView }) {
  return <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900"><div className="border-b border-zinc-200 p-5 dark:border-zinc-800"><h2 className="font-serif text-lg font-bold">Advertisements</h2><p className="mt-1 text-sm text-zinc-500">Creative, targeting, redirect, and visibility at a glance.</p></div>
    <div className="hidden overflow-x-auto xl:block"><table className="w-full min-w-[1180px] text-left text-sm"><thead className="bg-zinc-50 dark:bg-zinc-950"><tr>{['Preview','Title','Placement','Status','Target Summary','Redirect URL','Schedule','Priority','Actions'].map((heading) => <th key={heading} className="p-4 text-xs uppercase tracking-wide text-zinc-500">{heading}</th>)}</tr></thead><tbody>{items.map((ad) => <AdvertisementRow key={ad.id} ad={ad} onEdit={onEdit} onView={onView} />)}{items.length === 0 && <tr><td colSpan="9" className="p-10 text-center text-zinc-500">No advertisements have been created yet.</td></tr>}</tbody></table></div>
    <div className="grid gap-4 p-4 xl:hidden">{items.map((ad) => <AdvertisementCard key={ad.id} ad={ad} onEdit={onEdit} onView={onView} />)}{items.length === 0 && <p className="p-8 text-center text-zinc-500">No advertisements have been created yet.</p>}</div>
  </section>;
}

function AdvertisementRow({ ad, onEdit, onView }) { const placement = formatAdvertisementPlacement(ad.placement, ad.sidebar_side); const status = formatAdvertisementStatus(ad); const target = formatAdvertisementTargetSummary(ad.targets); return <tr className="border-t align-top dark:border-zinc-800"><td className="p-4"><AdThumbnail ad={ad} /></td><td className="max-w-48 p-4"><p className="font-bold text-zinc-950 dark:text-white">{ad.title}</p>{ad.alt_text && <p className="mt-1 text-xs text-zinc-500">Alt: {ad.alt_text}</p>}</td><td className="p-4"><p className="font-semibold">{placement.label}</p><p className="mt-1 text-xs text-zinc-500">{placement.helper}</p></td><td className="p-4"><StatusBadge status={status} /></td><td className="max-w-64 p-4"><TargetSummary summary={target} /></td><td className="max-w-52 p-4"><RedirectSummary ad={ad} /></td><td className="p-4"><ScheduleSummary ad={ad} /></td><td className="p-4"><p className="font-bold">Priority {ad.priority}</p><p className="mt-1 text-xs text-zinc-500">Higher shows first</p></td><td className="p-4"><ActionButtons ad={ad} onEdit={onEdit} onView={onView} /></td></tr>; }
function AdvertisementCard({ ad, onEdit, onView }) { const placement = formatAdvertisementPlacement(ad.placement, ad.sidebar_side); const status = formatAdvertisementStatus(ad); const target = formatAdvertisementTargetSummary(ad.targets); return <article className="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-700"><div className="flex gap-4"><AdThumbnail ad={ad} /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-bold">{ad.title}</h3>{ad.alt_text && <p className="mt-1 text-xs text-zinc-500">Alt: {ad.alt_text}</p>}</div><StatusBadge status={status} /></div></div></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><InfoBlock label="Target"><TargetSummary summary={target} /></InfoBlock><InfoBlock label="Placement"><p className="font-semibold">{placement.label}</p><p className="text-xs text-zinc-500">{placement.helper}</p></InfoBlock><InfoBlock label="Redirect"><RedirectSummary ad={ad} /></InfoBlock><InfoBlock label="Schedule"><ScheduleSummary ad={ad} /></InfoBlock></div><div className="mt-4 flex items-center justify-between border-t border-zinc-200 pt-4 dark:border-zinc-700"><span className="text-sm font-bold">Priority {ad.priority}</span><ActionButtons ad={ad} onEdit={onEdit} onView={onView} /></div></article>; }

function AdvertisementDetail({ ad, onClose, onEdit }) { const placement = formatAdvertisementPlacement(ad.placement, ad.sidebar_side); const status = formatAdvertisementStatus(ad); const target = formatAdvertisementTargetSummary(ad.targets); return <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><button aria-label="Close advertisement details" onClick={onClose} className="absolute inset-0 bg-zinc-950/60 backdrop-blur-sm" /><section role="dialog" aria-modal="true" aria-labelledby="advertisement-detail-title" className="relative max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl dark:bg-zinc-900"><header className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-200 bg-white px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900"><div><p className="text-xs font-bold uppercase tracking-widest text-amber-700">Advertisement details</p><h2 id="advertisement-detail-title" className="mt-1 font-serif text-xl font-bold">{ad.title}</h2></div><button onClick={onClose} className="rounded-lg p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"><X className="h-5 w-5" /></button></header><div className="space-y-6 p-6">{ad.image_url ? <div className="flex min-h-52 items-center justify-center rounded-2xl bg-zinc-950 p-4"><img src={ad.image_url} alt={ad.alt_text || ad.title} className="max-h-96 max-w-full object-contain" /></div> : <div className="flex min-h-52 items-center justify-center rounded-2xl bg-zinc-100 text-zinc-400 dark:bg-zinc-800"><ImageOff className="h-10 w-10" /></div>}<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Summary label="Alt text" value={ad.alt_text || 'Not provided'} /><Summary label="Placement" value={`${placement.label} · ${placement.helper}`} />{ad.placement === 'sidebar_sticky' && ad.sidebar_side && <Summary label="Sidebar side" value={ad.sidebar_side === 'right' ? 'Right Side' : 'Left Side'} />}<div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-700"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Status</p><div className="mt-2"><StatusBadge status={status} /></div></div><Summary label="Priority" value={String(ad.priority)} /><Summary label="Redirect" value={ad.redirect_url || 'No redirect'} /><Summary label="Open behavior" value={ad.redirect_url && ad.open_in_new_tab ? 'Opens in new tab' : 'Same tab or no redirect'} /><Summary label="Created by" value={ad.created_by?.name || 'Unknown'} /><Summary label="Created" value={formatDate(ad.created_at)} /><Summary label="Updated" value={formatDate(ad.updated_at)} /></div><InfoBlock label="Schedule"><ScheduleSummary ad={ad} /></InfoBlock><div><h3 className="font-serif text-lg font-bold">Targets</h3><p className="mt-1 font-semibold">{target.heading}</p>{target.lines.map((line) => <p key={line} className="text-sm text-zinc-500">{line}</p>)}<div className="mt-4 grid gap-3 sm:grid-cols-2">{ad.targets.map((item) => <div key={item.id} className="rounded-xl border border-zinc-200 p-4 text-sm dark:border-zinc-700"><p className="font-bold">{item.publication_name ? `${item.publication_label} · ${item.publication_name}` : item.page_label || 'Website page'}</p><p className="mt-1 text-zinc-500">{item.article_title || item.page_label || item.target_mode.replaceAll('_', ' ')}</p></div>)}</div></div></div><footer className="flex justify-end gap-3 border-t border-zinc-200 p-5 dark:border-zinc-800"><Button variant="outline" onClick={onClose}>Close</Button><Button onClick={onEdit} icon={Pencil}>Edit Advertisement</Button></footer></section></div>; }

function AdThumbnail({ ad }) { return ad.image_url ? <div className="flex h-[60px] w-24 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-zinc-100 dark:bg-zinc-800"><img src={ad.image_url} alt="" className="h-full w-full object-contain" /></div> : <div className="flex h-[60px] w-24 shrink-0 items-center justify-center rounded-lg bg-zinc-100 text-zinc-400 dark:bg-zinc-800"><ImageOff className="h-5 w-5" /></div>; }
function StatusBadge({ status }) { const tones = { green: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300', blue: 'bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300', amber: 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300', zinc: 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300' }; return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold capitalize ${tones[status.tone]}`}>{status.label}</span>; }
function TargetSummary({ summary }) { return <div><p className="font-bold">{summary.heading}</p>{summary.lines.map((line) => <p key={line} className="mt-1 text-xs text-zinc-500">{line}</p>)}</div>; }
function RedirectSummary({ ad }) { return ad.redirect_url ? <div><a href={ad.redirect_url} target="_blank" rel="noopener noreferrer" title={ad.redirect_url} className="block max-w-48 truncate font-semibold text-amber-700 hover:underline dark:text-amber-400">{ad.redirect_url}</a><p className="mt-1 text-xs text-zinc-500">{ad.open_in_new_tab ? 'Opens in new tab' : 'Opens in same tab'}</p></div> : <span className="text-sm text-zinc-500">No redirect</span>; }
function ScheduleSummary({ ad }) { return <div>{formatAdvertisementSchedule(ad).map((line) => <p key={line} className="text-xs font-semibold text-zinc-600 dark:text-zinc-300">{line}</p>)}</div>; }
function ActionButtons({ ad, onEdit, onView }) { return <div className="flex gap-2"><button onClick={() => onView(ad)} className="inline-flex items-center gap-1 rounded-lg border border-zinc-200 px-2.5 py-2 text-xs font-bold hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800"><Eye className="h-3.5 w-3.5" />View</button><button onClick={() => onEdit(ad)} className="inline-flex items-center gap-1 rounded-lg bg-amber-600 px-2.5 py-2 text-xs font-bold text-white hover:bg-amber-500"><Pencil className="h-3.5 w-3.5" />Edit</button></div>; }
function InfoBlock({ label, children }) { return <div><p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-zinc-400">{label}</p>{children}</div>; }
function formatDate(value) { return value ? new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)) : 'Not available'; }
...
function SelectField({ label, value, onChange, options, empty = '' }) { return <label className="block text-sm font-bold">{label}<SearchableSelect value={value} onChange={onChange} options={options} placeholder="Select…" clearable className="mt-2" />{empty && options.length === 0 && <span className="mt-2 block text-xs font-normal text-zinc-500">{empty}</span>}</label>; }
function DateTimeField({ label, value, onChange, minDate }) { return <label className="block text-sm font-bold">{label}<div className="mt-2"><FlatpickrInput mode="datetime" value={value} onChange={onChange} minDate={minDate} /></div></label>; }
