...
import { canStartDirectPublicationUpload, directPublicationIssueLabel, getOrCreateDraftOperation, restoredDirectPublicationStep } from './directPublicationUtils.mjs';
...
export default function DirectPublicationEditor({ articleId = null }) {
  ...
  const refresh = async (id = draftIdRef.current) => { if (!id) return; const response = await api.get(`/admin/direct-publications/${id}`); hydrate(response.data.data, response.data.readiness); };
  ...
  const uploadFile = async (event, purpose) => {
    ...
    setUploading(purpose); setUploadProgress(0); setErrors([]);
    try { const id = draftIdRef.current || await ensureDraft(); const upload = await uploadAndAwaitClean({ file, purpose, attachableId: Number(id), onProgress: setUploadProgress }); await api.post(`/admin/direct-publications/${id}/files`, { upload_id: upload.id, purpose }, { headers: { 'Idempotency-Key': `direct-file-attach:${upload.id}` } }); await refresh(id); toast(`${file.name} was scanned and attached.`, 'success'); }
  ...
  return <main className="mx-auto w-full max-w-7xl px-4 py-6 pb-24 sm:px-6 lg:px-8">
    ...
    <div className="space-y-5">
      ...
      {step === 3 && <><Card title="Upload publication files" description="Each upload is scanned before it joins this publication package."><div className="grid gap-3 md:grid-cols-5">{[['direct_publication_manuscript','Final PDF','application/pdf'],['direct_publication_figure','Figure','image/png,image/jpeg,image/webp'],['direct_publication_cover','Cover image','image/png,image/jpeg,image/webp'],['direct_publication_supplementary','Supplementary',''],['direct_publication_source','Source file','']].map(([purpose, label, accept]) => { const uploadAllowed = canStartDirectPublicationUpload({ magazineId: form.magazine_id, title: form.title, status, purpose }); return <label key={purpose} className={`rounded-xl border-2 border-dashed p-5 text-center text-sm font-bold ${uploadAllowed ? 'cursor-pointer text-[var(--primary)]' : 'cursor-not-allowed text-[var(--muted)]'}`}><FileUp className="mx-auto mb-2 h-5 w-5"/>{uploading === purpose ? `Uploading ${uploadProgress || 0}%…` : status === 'published' && purpose === 'direct_publication_manuscript' ? 'Replacement PDF' : label}<input type="file" className="sr-only" accept={accept} disabled={!uploadAllowed || Boolean(uploading)} onChange={(event) => uploadFile(event, purpose)}/></label>; })}</div></Card><Card title="Publication file package" description="Select the final PDF and explicitly control public, download, and package visibility."><div className="grid gap-7 lg:grid-cols-2"><FileGroup title="Final manuscripts" files={groups.manuscripts} primaryId={primaryId} onPrimary={selectPrimary} onRemove={removeFile} settings={form.file_settings} onSetting={setFileSetting} published={status === 'published'}/><FileGroup title="Figures and cover" files={groups.figures} primaryId={primaryId} onPrimary={selectPrimary} onRemove={removeFile} settings={form.file_settings} onSetting={setFileSetting} published={status === 'published'}/><FileGroup title="Supplementary files" files={groups.supplementary} primaryId={primaryId} onPrimary={selectPrimary} onRemove={removeFile} settings={form.file_settings} onSetting={setFileSetting} published={status === 'published'}/><FileGroup title="Internal source files" files={groups.sources} primaryId={primaryId} onPrimary={selectPrimary} onRemove={removeFile} settings={form.file_settings} onSetting={setFileSetting} published={status === 'published'}/></div></Card></>}
    ...
    <footer className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--border)] bg-[var(--surface)]/95 px-4 py-3 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><button type="button" disabled={step === 0 || busy} onClick={() => setStep((value) => value - 1)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[var(--border)] px-4 text-sm font-bold disabled:opacity-40"><ArrowLeft className="h-4 w-4"/>Back</button><div className="flex items-center gap-3"><p className="hidden text-xs font-semibold text-[var(--muted)] sm:block">Step {step + 1} of {STEPS.length}</p>{status === 'direct_publication_draft' && <button type="button" disabled={busy || Boolean(uploading) || Boolean(uploadingSection)} onClick={save} className="inline-flex min-h-10 items-center rounded-lg border border-[var(--border)] px-4 text-sm font-bold disabled:opacity-40">Save Draft</button>}</div>{step < STEPS.length - 1 ? <button type="button" disabled={busy || Boolean(uploading) || Boolean(uploadingSection)} onClick={next} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[var(--primary)] px-5 text-sm font-bold text-[var(--primary-foreground)] disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin"/> : null}{busy ? 'Saving…' : 'Next'}{!busy && <ArrowRight className="h-4 w-4"/>}</button> : <Link href="/admin/direct-publications" className="inline-flex min-h-10 items-center rounded-lg border border-[var(--border)] px-5 text-sm font-bold">Return to Dashboard</Link>}</div></footer>
