...
function FileGroup({ title, files, primaryId, onPrimary, settings, onSetting, published }) {
...
export default function DirectPublicationEditor({ articleId = null }) {
  ...
  const hydrate = useCallback((data, blockers) => {
    ...
    const fileSettings = Object.fromEntries((data.files || []).map((file) => [file.id, file.metadata?.publication_visibility || {}]));
  ...
  const groups = useMemo(() => ({
    ...
    figures: files.filter((file) => ['direct_publication_figure', 'direct_publication_cover'].includes(file.file_type)),
  ...
  const payload = () => ({ ...form, magazine_id: Number(form.magazine_id), magazine_issue_id: form.magazine_issue_id ? Number(form.magazine_issue_id) : null,
  ...
  const save = async () => {
    ...
    return Boolean(await run(() => status === 'published'
      ? api.post(endpoint, payload(), { headers: headers() })
      : api.put(endpoint, payload(), { headers: headers() }), status === 'published' ? 'Published content and metadata corrected with an audit record.' : 'Publication draft saved.'));
  };
  const action = (path, data, success) => run(() => api.post(`/admin/direct-publications/${articleId}/${path}`, data, { headers: headers() }), success);
  ...
  const uploadSectionImage = async (section, file) => {
    ...
    try { const upload = await uploadAndAwaitClean({ file, purpose: 'publication_section_image', attachableId: Number(articleId) }); updateSection(section.client_id, { media_upload_session_id: upload.id, preview_url: preview }); toast('Section image uploaded and scanned.', 'success'); }
  ...
  const uploadFile = async (event, purpose) => {
    const file = event.target.files?.[0]; if (!file || !articleId) return;
    setUploading(purpose); setErrors([]);
    try { const upload = await uploadAndAwaitClean({ file, purpose, attachableId: Number(articleId) }); await api.post(`/admin/direct-publications/${articleId}/files`, { upload_id: upload.id, purpose }, { headers: headers() }); await refresh(); toast(`${file.name} was scanned and attached.`, 'success'); }
    catch (error) { setErrors([error.userMessage || safeApiMessage(error, 'File upload failed.')]); }
