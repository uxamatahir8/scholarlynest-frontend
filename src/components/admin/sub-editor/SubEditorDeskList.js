...
export default function SubEditorDeskList({
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        <LoadingState label="Loading assigned manuscripts..." className="min-h-[320px]" />
      ) : error ? (
        <ErrorState title="Assigned manuscripts could not be loaded">{error}</ErrorState>
      ) : assignments.length === 0 ? (
        <EmptyState title="No assigned manuscripts found">
                  ...
                  <div className="min-w-0 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-md border border-[var(--border)] bg-[var(--surface-muted)] px-2 py-1 text-xs font-bold uppercase tracking-wide text-[var(--muted)]">
                        Task: {statusLabel(assignment.status)}
                      </span>
                      <span className="text-xs font-medium text-[var(--muted)]">
                        Manuscript status: {statusLabel(article.status)}
                      </span>
      ...
      <Pagination
        ...
        label="Assigned manuscript pages"
      />
    </div>
  );
}
