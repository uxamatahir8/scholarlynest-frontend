...
const primaryActionLabel = (assignment) => {
  if (assignment.primary_action === 'continue_screening') return 'Continue Screening';
  if (assignment.primary_action === 'manage_reviewers') return 'Manage Reviewers';
  if (assignment.primary_action === 'review_reviewer_progress') return 'Review Reviewer Progress';
  if (assignment.primary_action === 'submit_recommendation') return 'Submit Recommendation';
  if (assignment.primary_action === 'view_recommendation') return 'View Recommendation';
  return 'Continue Screening';
};

export default function SubEditorDeskList({
  observerMode = false,
  observerUser = null,
  observerParams = {},
}) {
  ...
  const loadAssignments = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await api.get('/admin/my-sub-editor-assignments', { params: queryParams });
      ...
      logError('Failed to load sub editor assignments:', err);
      setError(safeApiMessage(err, 'Unable to load assigned manuscripts.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <ConsolePageHeader
        title={observerMode && observerUser ? `${observerUser.name}'s Assigned Manuscripts` : 'Assigned Manuscripts'}
        description="Review and progress manuscripts assigned to you."
      />
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div>
            ...
            >
              <option value="active">Active Manuscripts</option>
              <option value="pending">Pending Recommendation</option>
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium text-[var(--muted)]">
              Showing {start}-{end} of {meta.total} manuscripts.
            </span>
            <button
              type="button"
      ...
      {error && (
        <Alert tone="danger" title="Assigned manuscripts could not be loaded">
          {error}
        </Alert>
      )}

      {loading && assignments.length === 0 ? (
        <LoadingState label="Loading assigned manuscripts..." className="min-h-[280px]" />
      ) : assignments.length === 0 ? (
        <EmptyState title="No assigned manuscripts found" className="py-12">
          There are no assigned manuscripts matching the selected filter or search.
        </EmptyState>
      ) : (
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-bold uppercase tracking-wider ${
                        ...
                        Task: {assignment.status === 'completed' || assignment.completed_at ? 'Completed' : 'Active'}
                      </span>

                      <span className="text-xs text-[var(--muted)]">
                        Manuscript status: <span className="font-semibold text-[var(--foreground)]">{article.status?.replaceAll('_', ' ')}</span>
                      </span>
      ...
      <Pagination
        ...
        label="Sub Editor assignment pages"
      />
    </div>
  );
}
