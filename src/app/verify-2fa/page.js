...
import { AlertCircle, KeyRound, Loader2, Mail, ShieldCheck, Smartphone } from 'lucide-react';
...
function VerifyMfaForm() {
  ...
  useEffect(() => {
    try {
      ...
      if (!stored?.token || !Array.isArray(stored.methods)) {
        router.replace('/login');
        return;
      }
      setChallenge(stored);
      setMethod(stored.defaultMethod || stored.methods[0]);
    } catch {
      router.replace('/login');
    }
  }, [router]);

  const switchMethod = async (next) => {
    setError('');
    setCode('');
    if (next === 'email' && method !== 'email') {
      try {
        setLoading(true);
        await api.post('/auth/mfa/email/resend', { challenge_token: challenge.token });
        toast('Authentication code sent to your email.', 'success');
      } catch (err) {
        const message = safeApiMessage(err, 'Unable to send an email code.');
        setError(message);
        return;
      } finally {
        setLoading(false);
  ...
  const methods = [...challenge.methods, ...(challenge.methods.includes('totp') ? ['recovery_code'] : [])];
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-border bg-surface p-8 shadow-md dark:bg-[#121316]">
      ...
      {methods.length > 1 && <div className="grid gap-2 sm:grid-cols-2">
        {methods.map((item) => <button key={item} type="button" onClick={() => switchMethod(item)} disabled={loading} className={`rounded-lg border px-3 py-2 text-xs font-bold ${method === item ? 'border-amber-500 bg-amber-50 text-amber-900 dark:bg-amber-500/10 dark:text-amber-300' : 'border-border text-muted'}`}>{labels[item]}</button>)}
      </div>}
