...
import {
  Plus, Trash2, Edit3, X, Loader2, AlertCircle, HelpCircle, ArrowLeft, Check, BookOpen
} from 'lucide-react';
import { Button } from '../../../../components/ui/Button';
import { ConfirmationModal } from '../../../../components/ui/ConfirmationModal';
import Link from 'next/link';

export default function ArticleTypeManagement() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/admin/article-types/${editingId}`, payload);
        toast('Article Type updated successfully!', 'success');
      } else {
        await api.post('/admin/article-types', payload);
        toast('Article Type created successfully!', 'success');
      }
  ...
  const handleDelete = async (id) => {
    try {
      setDeletingId(id);
      await api.delete(`/admin/article-types/${id}`);
  ...
  const handleToggleActive = async (type) => {
    try {
      await api.put(`/admin/article-types/${type.id}`, {
  ...
  if (authLoading || (loading && types.length === 0)) {
    return (
  ...
  return (
    <div className="p-6 sm:p-8 w-full space-y-6 animate-in fade-in duration-300 text-left">
      <title>Manage Article Types - ScholarlyNest</title>
      ...
      <div className="flex items-center justify-between pb-4 border-b border-zinc-200/80 dark:border-zinc-800">
        ...
        <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">
          ...
          <span>Settings</span>
          <span>/</span>
          <span className="text-[var(--accent-gold)]">Article Types</span>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--accent-gold)] flex items-center gap-1">
              <BookOpen className="w-3.5 h-3.5" /> Article Types System
            </span>
            ...
            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 font-medium mt-1">
              Add and update administrative classifications for newly submitted manuscripts.
            </p>
          </div>
          <div>
            {!isFormOpen && (
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Describe this article type..."
      ...
      {types.length === 0 ? (
        <div className="text-center py-16 rounded-2xl border border-dashed border-zinc-350 dark:border-zinc-850 p-8 glass-panel">
          ...
          <h3 className="font-serif text-lg font-bold text-zinc-700 dark:text-zinc-300">No Article Types Configured</h3>
          <p className="text-xs text-[var(--muted)] mt-1 font-medium max-w-sm mx-auto">
            Get started by adding your first article type classification.
          </p>
        </div>
      ) : (
        <div className="glass-panel overflow-hidden border border-zinc-200/80 dark:border-zinc-800 rounded-2xl bg-white/70 dark:bg-black/10 shadow-sm">
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800 text-left">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 font-medium text-xs">
              {types.map((type) => (
                <tr key={type.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors">
                  <td className="px-6 py-4 font-bold text-zinc-900 dark:text-white">{type.name}</td>
                  <td className="px-6 py-4 text-zinc-550 dark:text-zinc-400 max-w-xs truncate">{type.description || <em className="text-zinc-400">None</em>}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <Button variant="secondary" size="sm" onClick={() => handleEdit(type)} icon={Edit3}>
                      Edit
                    </Button>
                    {canDeleteRecords && (
                      <Button
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this article type? This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
        onConfirm={() => {
          if (confirmId) {
