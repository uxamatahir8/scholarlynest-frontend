...
export default function SubjectAreaManagement() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/admin/subject-areas/${editingId}`, payload);
        toast('Subject Area updated successfully!', 'success');
      } else {
        await api.post('/admin/subject-areas', payload);
        toast('Subject Area created successfully!', 'success');
      }
  ...
  const handleDelete = async (id) => {
    try {
      setDeletingId(id);
      await api.delete(`/admin/subject-areas/${id}`);
  ...
  const handleToggleActive = async (area) => {
    try {
      await api.put(`/admin/subject-areas/${area.id}`, {
  ...
  if (authLoading || (loading && areas.length === 0)) {
    return (
  ...
  return (
    <div className="p-6 sm:p-8 w-full space-y-6 animate-in fade-in duration-300 text-left">
      <title>Manage Subject Areas - ScholarlyNest</title>
      ...
      <div className="flex items-center justify-between pb-4 border-b border-zinc-200/80 dark:border-zinc-800">
        ...
        <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest text-[var(--muted)]">
          ...
          <span>Settings</span>
          <span>/</span>
          <span className="text-[var(--accent-gold)]">Subject Areas</span>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--accent-gold)] flex items-center gap-1">
              <BookOpen className="w-3.5 h-3.5" /> Subject Areas System
            </span>
            ...
            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 font-medium mt-1">
              Add and update administrative academic subject areas.
            </p>
          </div>
          <div>
            {!isFormOpen && (
        ...
        <div className="glass-panel p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-lg bg-zinc-50/50 dark:bg-zinc-900/30 animate-in slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between pb-3 border-b border-zinc-200/60 dark:border-zinc-800/80 mb-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--foreground)]">
              {editingId ? 'Edit Subject Area' : 'New Subject Area'}
            </h3>
      ...
      {areas.length === 0 ? (
        <div className="text-center py-16 rounded-2xl border border-dashed border-zinc-350 dark:border-zinc-850 p-8 glass-panel">
          ...
          <h3 className="font-serif text-lg font-bold text-zinc-700 dark:text-zinc-300">No Subject Areas Configured</h3>
          <p className="text-xs text-[var(--muted)] mt-1 font-medium max-w-sm mx-auto">
            Get started by adding your first subject area classification.
          </p>
        </div>
      ) : (
        <div className="glass-panel overflow-hidden border border-zinc-200/80 dark:border-zinc-800 rounded-2xl bg-white/70 dark:bg-black/10 shadow-sm">
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800 text-left">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 font-medium text-xs">
              {areas.map((area) => (
                <tr key={area.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors">
                  <td className="px-6 py-4 font-bold text-zinc-900 dark:text-white">{area.name}</td>
                  <td className="px-6 py-4 text-zinc-555 dark:text-zinc-400 max-w-xs truncate">{area.description || <em className="text-zinc-400">None</em>}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <Button variant="secondary" size="sm" onClick={() => handleEdit(area)} icon={Edit3}>
                      Edit
                    </Button>
                    {canDeleteRecords && (
                      <Button
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this subject area? This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
        onConfirm={() => {
          if (confirmId) {
