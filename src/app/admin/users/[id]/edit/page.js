...
import {
  UserCheck, ArrowLeft, Loader2, AlertTriangle, ShieldAlert, ChevronDown, X
} from 'lucide-react';
...
export default function EditUserPage() {
  ...
  // Load User Details
  useEffect(() => {
    if (!authLoading && isSuperAdmin && userId) {
      const fetchUserDetails = async () => {
        ...
        try {
          ...
          setName(u.name || '');
          setEmail(u.email || '');
          setUniversityName(u.university || '');
          setStatus(u.status || 'active');
          if (u.roles && u.roles.length > 0) {
            setRoleId(u.roles[0].id.toString());
          } else {
            setRoleId('');
          }
          if (u.assigned_editors) {
            setSelectedEditorIds(u.assigned_editors.map(e => e.id));
          }
        } catch (err) {
          if (err.response && err.response.status === 404) {
            setGeneralError('The requested user account was not found.');
          } else {
            setGeneralError('We encountered a problem loading details for this user.');
          }
  ...
  const handleSubmit = async (e) => {
    ...
    // Password is only validated if entered (optional on edit)
    if (password) {
    ...
    try {
      ...
      if (response && response.status === 422) {
        ...
        toast('Account modifications validation failed.', 'error');
      } else {
        setGeneralError('We encountered an error trying to save these changes. Please verify connectivity and try again.');
        toast('An unexpected error occurred during user update.', 'error');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (authLoading || (!isSuperAdmin && authUser) || loadingOptions || (loadingUser && !generalError)) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        <Loader2 className="w-10 h-10 animate-spin text-amber-600 dark:text-amber-400" />
        <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest font-mono">Loading Editor Portal...</span>
      </div>
    );
  }
      ...
      <div className="p-6 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/40 rounded-xl flex items-start space-x-4 animate-in fade-in slide-in-from-bottom-4">
        ...
        <div>
          ...
          <p className="text-xs text-red-600 dark:text-red-300 mt-1">
            You must possess Super Admin privileges to access the user configuration panels.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--muted)] mt-1.5 font-medium">
            Modify personnel details, update access controls, or sync editorial assignments.
          </p>
        </div>
      </div>
      ...
      <Card className="border border-[var(--muted-border)] bg-[var(--card-bg)] shadow-md">
        <CardHeader className="border-b border-[var(--muted-border)]/50 pb-4">
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-xs font-bold uppercase tracking-widest text-[var(--foreground)]">Personnel Management</CardTitle>
          </div>
          <CardDescription className="text-xs">
            Modify credentials and platform configuration below. Password updates are optional.
          </CardDescription>
        </CardHeader>

        <CardContent className="p-6">
          {generalError ? (
            <div className="py-10 px-4 text-center flex flex-col items-center justify-center space-y-4">
              <AlertTriangle className="w-10 h-10 text-amber-500" />
              <h4 className="text-sm font-bold text-[var(--foreground)]">Configuration Blocked</h4>
              <p className="text-xs text-[var(--muted)] max-w-md">{generalError}</p>
              <Link href="/admin/users" passHref legacyBehavior>
                <Button variant="outline" size="sm" className="mt-2 text-xs font-bold py-2 border-[var(--muted-border)] cursor-pointer">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">Full Name</label>
                  <input
                    type="text"
                    value={name}
                  ...
                  />
                  {fieldErrors.name && (
                    <p className="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wider font-mono">{fieldErrors.name}</p>
                  )}
                </div>

                {/* Email */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">Email Address</label>
                  <input
                    type="email"
                    value={email}
                  ...
                  />
                  {fieldErrors.email && (
                    <p className="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wider font-mono">{fieldErrors.email}</p>
                  )}
                </div>

                {/* Role Selection */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">Access Role</label>
                  <select
                    value={roleId}
                  ...
                  {fieldErrors.role_id && (
                    <p className="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wider font-mono">{fieldErrors.role_id}</p>
                  )}
                </div>

                {/* University/Organization */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">University / Organization</label>
                  <input
                    type="text"
                ...
                {/* Account Status */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">Account Status</label>
                  <select
                    value={status}
                    onChange={(e) => setStatus(e.target.value)}
                    className="w-full text-xs font-semibold px-3 py-2 bg-[var(--card-bg)] border border-[var(--muted-border)] rounded-xl focus:outline-none focus:border-amber-500 transition-colors text-[var(--foreground)]"
                  >
                ...
                {/* Optional Password Section Informative Notice */}
                <div className="md:col-span-2 py-1 px-3 border border-amber-500/20 bg-amber-500/5 rounded-lg text-[10px] text-amber-600 dark:text-amber-400 font-bold uppercase tracking-wide select-none">
                  Security Update: Enter a new password only if you wish to reset the user credentials. Leave blank to preserve active login password.
                </div>

                {/* Password */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">New Password</label>
                  <input
                    type="password"
                    value={password}
                  ...
                  {fieldErrors.password && (
                    <p className="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wider font-mono">{fieldErrors.password}</p>
                  )}
                </div>

                {/* Confirm Password */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] font-mono">Confirm New Password</label>
                  <input
                    type="password"
                  ...
                  {fieldErrors.confirm_password && (
                    <p className="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wider font-mono">{fieldErrors.confirm_password}</p>
                  )}
                </div>
              </div>
              ...
              {isSubEditor && (
                <div className="border border-[var(--muted-border)] rounded-xl p-4 bg-[var(--foreground)]/5 space-y-4 animate-in slide-in-from-top-2 duration-300">
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-[var(--foreground)] font-mono">Assigned Editor(s)</h3>
                  ...
                  {fieldErrors.editor_ids && (
                    <p className="text-[10px] font-bold text-red-500 uppercase tracking-wider mt-1 font-mono">{fieldErrors.editor_ids}</p>
                  )}
                </div>
              )}
