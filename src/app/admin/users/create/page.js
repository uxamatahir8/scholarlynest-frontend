...
import {
  ArrowLeft, Save, Loader2, UserPlus
} from 'lucide-react';
...
export default function CreateUserPage() {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      toast(`User ${name} created successfully. Welcome credentials email dispatched.`, 'success');
  ...
  if (authLoading || loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <span className="text-xs font-bold text-zinc-405 uppercase tracking-widest font-mono">Loading Creation Context...</span>
      </div>
    );
  }
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-[var(--muted)] mt-1 font-medium">
            Create a verified account without a password. Welcome login link email will be dispatched.
          </p>
        </div>
      </div>
